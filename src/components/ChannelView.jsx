...
import { channelService, videoService, shortsService, followService, notificationService } from '../services/puterService';

const ChannelView = ({ user, channelId, onEdit, onNavigate }) => {
    ...
    const loadChannel = async () => {
        ...
        try {
            ...
            if (ch) {
                ...
                if (user && user.mode !== 'guest' && user.id !== ch.ownerId) {
    ...
    const handleFollowToggle = async () => {
        ...
        try {
            const result = await followService.toggleFollow(user.id, channel.id);
            setIsFollowing(result.following);
            setChannel(prev => ({
                ...prev,
                followersCount: Math.max(0, (prev.followersCount || 0) + (result.following ? 1 : -1))
            }));

            // Send Notification ifFollowed
            if (result.following) {
                await notificationService.push(channel.ownerId, {
                    type: 'follow',
                            ...
                            <>
                                ...
                                <div className="channel-stats">
                                    <span className="channel-stat">
                                        <Users size={16} />
                                        {channel.followersCount || 0} متابع
                                    </span>
                                    <span className="channel-stat">
                                        <Video size={16} />
                                        {channel.videosCount || 0} فيديو
                                    </span>
                                    <span className="channel-stat">
                                        <Film size={16} />
                                        {channel.shortsCount || 0} مقطع
                                    </span>
                                </div>
                            </>
                        )}
